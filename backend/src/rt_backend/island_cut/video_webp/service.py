"""MP4 → 透明 Animated WebP 算法 + 全流水线（自完备）。

算法五要素（与 video_island.service 一致；本文件不 import 它保持横向隔离）：
  1. 边框中位 bg（对角落水印鲁棒）
  2. diff > tol + 闭运算 + 连通域 + 仅最大分量 + fill_holes
  3. 全帧 union bbox + pad
  4. NEAREST 缩放（LANCZOS 在二值 alpha 上振铃会桥接远端小岛）
  5. PIL Animated WebP：lossless=True 保 alpha + save_all
"""
...
def make_preview_png(
    ...
    """首/中/尾三帧棋盘格 preview（alpha_composite 在棋盘上叠 RGBA）。"""
