"""MP4 → 透明 Animated WebP 端点（/api/island-cut/video-webp/*）。"""
...
from .store import IslandVideoWebPJobStore, VideoJob
...
def _job_or_404(job_id: str, store: IslandVideoWebPJobStore) -> VideoJob:
...
def build_webp_router(store_provider) -> APIRouter:
    router = APIRouter(prefix="/api/island-cut/video-webp", tags=["island-cut-video-webp"])

    def _store(request: Request) -> IslandVideoWebPJobStore:
    ...
    def create_job(
        file: bytes = File(...),
        params: str = Form("{}"),
        store: IslandVideoWebPJobStore = Depends(_store),
    ):
        try:
    ...
    @router.get("/jobs/{job_id}/webp")
    def get_webp(job_id: str, store: IslandVideoWebPJobStore = Depends(_store)):
        job = _job_or_404(job_id, store)
        return FileResponse(job.webp_path, media_type="image/webp")

    @router.get("/jobs/{job_id}/preview.png")
    def get_preview(job_id: str, store: IslandVideoWebPJobStore = Depends(_store)):
        job = _job_or_404(job_id, store)
        return FileResponse(job.preview_path, media_type="image/png")

    @router.delete("/jobs/{job_id}")
    def delete_job(job_id: str, store: IslandVideoWebPJobStore = Depends(_store)):
