"""MP4→APNG 任务的临时持久化（与 IslandVideoJobStore 同机制）。"""
...
class IslandVideoApngJobStore:
    ...
    def create(self, result: VideoResult, apng: bytes, preview: bytes) -> VideoJob:
        self._prune()
        job_id = uuid.uuid4().hex[:12]
        job_dir = self._root / job_id
        job_dir.mkdir(parents=True, exist_ok=True)
        apng_path = job_dir / APNG_NAME
        prev_path = job_dir / PREVIEW_NAME
        apng_path.write_bytes(apng)
        prev_path.write_bytes(preview)
        job = VideoJob(
            id=job_id, dir=job_dir, apng_path=apng_path, preview_path=prev_path,
