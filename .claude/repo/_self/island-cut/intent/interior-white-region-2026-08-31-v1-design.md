# Intent: 内部白区「检测→编号→勾选重切」预测设计（v1）
...
相关文档：[interior-white-region-2026-08-31-intent.md](./interior-white-region-2026-08-31-intent.md) · [../project/compare_matting-libraries-2026-08-31-v1-status.md](../project/compare_matting-libraries-2026-08-31-v1-status.md)
...
POST apply(hole_ids) → store 取原图字节（场景 1 落盘的副本）→ 重新 load + 重跑 run_cut，fg 扣除勾选洞的 union mask → 覆盖 job 的 pieces/holes → 返回与 create 同构的 CutResponse。每次 apply 从原图出发 → 接口幂等，可撤销（改勾选集合再 apply 即可）。
...
4. **原图字节落盘换可重切性**。理由：store 现不存原图（store.py:36-52，compare 差距 5），驻留内存则多 job 并发下极易顶破 300MB；落盘与 TTL 清理天然对齐。备选存 fg mask 的 .npy：读取快但体积更大，一期不取。
5. **apply = 全量重切 + 幂等**。理由：复用既有流程正确性，实现最小；增量重切（只重算受影响岛）留二期。
6. **洞编号复用 `_order_reading` 阅读序**。理由：与岛屿编号（island_NN）体验一致。
7. **MAX_PIXELS 建议收紧至 ~12MP**。理由：40MP 下 label 的 int32 中间数组即 ~160MB，叠加副本与 zip 打包可破 300MB；12MP 时各中间数组 ≤48MB 量级（估）。此项涉及行为变化，列入待拍板。
8. **不引入 AI 抠图库（远期备注）**：若未来复杂背景成为真实输入，compare 文档结论下唯一预算内路径为 rembg+u2netp 独立容器（单独 mem_limit、模型 COPY 进镜像）或离线预生成，均为独立议题，不混入本期。
...
- 防蔓延：不做洞的交互式编辑器逻辑、不做增量重切、不做 AI 检测路径——均为二期/独立议题。
...
- **M3（可选）**：MAX_PIXELS 收紧 + 前端序号勾选交互（前端另行仓库/实施）。
...
| 5 | 4MP 输入全流程峰值 RSS < 300MB | 容器内实测（/usr/bin/time -v 或 docker stats） |
...
| # | 决策 | 推荐 |
| --- | --- | --- |
| 1 | apply 结果覆盖同 job_id 还是生成新 job | 覆盖同 job（pieces/holes 全量替换，URL 不变） |
| 2 | apply 语义：全量勾选幂等 vs 增量切换 | 全量幂等（可撤销，实现简单） |
| 3 | hole_min_area 默认值 | 200px（可调，0 关闭检测） |
| 4 | MAX_PIXELS 是否收紧 | 收紧至 ~12MP，或按验收 5 的实测峰值定 |
| 5 | bg_threshold 复用现状 235 还是洞单独阈值 | 复用 235（一期不加新阈值） |

## 十一、参考

- 代码：backend/src/rt_backend/island_cut/service.py:59-64（泛洪）、:161-201（run_cut）、router.py:24-25（上限）、router.py:94-105（内存 zip）、store.py:36-52（不落原图）
- 本主题文档：intent/interior-white-region-2026-08-31-intent.md · project/compare_matting-libraries-2026-08-31-v1-status.md
